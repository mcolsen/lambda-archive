...
function get20s(array){
	...
	{
		let parsedArtist = reformatArtist(array[i]);
		if (parsedArtist.birth >= 1900 && parsedArtist.death <= 2000){
			parsed.push(parsedArtist.name);
		}
	}
	return parsed;

	function reformatArtist(artist){
		let newArtist = {name: artist.name};
		let yearsArray = artist.years.split(' - ');
		newArtist.birth = yearsArray[0];
		newArtist.death = yearsArray[1];
		return newArtist;
	}
}
...
function getHTML(array){
	let arrayCopy = [...array];
	for(const artist of arrayCopy){
		artist.imgurl = artist.name + ".jpg";
		for(let i = 0; i < 5; i++){
			artist.imgurl = artist.imgurl.replace(" ", "-");
		}
		console.log(`
			<div class="artist">
			<div class="image">
				\t<img src="assets/${artist.imgurl}"/>
			</div>
...
function randomize(array){
	...
	while(randomized.length < maxIndex){
		index = Math.floor(Math.random() * arrayCopy.length);
		randomized.push(arrayCopy[index]);
