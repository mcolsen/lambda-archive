...
function is31Flavors(arrayToValidate){
    if(arrayToValidate.length === 31){
        return true; 
    }
    else{
        return false; 
...
function addFlavor(flavorArray, flavor){
    console.log(flavorArray.unshift(flavor));
    return flavorArray;
}
...
function getFlavorByIndex(flavorArray, i){
    return flavorArray[i];
}
...
function removeFlavorByName(flavorArray, flavorToRemove){
    if(flavorArray.includes(flavorToRemove) !== true){
        console.log("Error - flavor not found");
        return flavorArray;
    }
    else{
        flavorArray.splice(flavorArray.indexOf(flavorToRemove), 1);
        return flavorArray;
    }
}
...
function filterByWord(flavorArray, search){
    ...
    for(let i = 0; i < flavorArray.length; i++){
        if (flavorArray[i].includes(search) === true){
...
function getAverageWordLength(arrayToSearch){
    let arrayOfLengths = [];
    for(let i = 0; i < arrayToSearch.length; i++){
        let subdividedString = [];
        for(let j = 0; j < arrayToSearch[i].length; j++){
            subdividedString.push(arrayToSearch[i].charAt(j));
        }
        let spaceCounter = 0; 
        for(let j = 0; j < subdividedString.length; j++){
            if(subdividedString[j] === " "){
                spaceCounter++;
            }
        }
        ...
        arrayOfLengths.push(spaceCounter + 1);
    }
    let sumLengths = 0; 
    for(let i = 0; i < arrayOfLengths.length; i++){
        sumLengths += arrayOfLengths[i];
    }
    return (sumLengths / arrayOfLengths.length);
}
...
function getRandomFlavors(array1, array2, array3, array4){
    let combinedArray = [];
    addArrayContents(array1);
    addArrayContents(array2);
    addArrayContents(array3);
    addArrayContents(array4);
    let randomizedArray = [];
    while(randomizedArray.length < 31){
        randomizedArray.push(randomSelection(combinedArray));
    }
    return randomizedArray;

    function addArrayContents(arrayToAdd){
        for(let i = 0; i < arrayToAdd.length; i++){
            combinedArray.push(arrayToAdd[i]);
        }
    }

    function randomSelection(arrayToSearch){
        let index = randomIndex();
        let selectedFlavor = arrayToSearch[index];
...
function randomSelection(arrayToSearch){
    let index = randomIndex();
    let selectedFlavor = arrayToSearch.splice(index, 1);
